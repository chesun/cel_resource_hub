...
- [x] Link audit (2nd pass): every repo's Documentation map links its artifacts to GitHub (20 links) + the public predecessor `caschls`. `va_paper_clone` + `cde_va_project_fork` are 404 → left unlinked — 2026-06-22
- [x] Link audit (1st pass): 5 high-value cross-links (homepage git/Claude Code; va_consolidated first-mentions; ADR cross-links). 3 agent-flagged candidates were false positives, verified + skipped — 2026-06-22
- [x] External links open in a new tab (`mkdocs-open-in-new-tab` plugin); internal unaffected, instant-nav-aware — 2026-06-22
- [x] Added a "What's a decision record (ADR)?" explainer on the Offboarding page (ADRs referenced 8× but never defined) — 2026-06-22
- [x] Reproducible pipelines: added a `main.do` skeleton + toggle walkthrough (grounded in va_consolidated) — 2026-06-22
- [x] Whole-hub page-by-page walkthrough: contacts → author-only; filled Scribe paths (calpads under `users/chesun/gsr/` post-restructuring); status fixes (csac_2024 + ccylc complete; va_consolidated "acceptance run in progress"; csac2025 offboarding); uniform `Name (repo)` naming; strawman + voice fixes; Google Docs out of scope — 2026-06-22
- [x] Claude Code intro links my workflow template [chesun/claude-research-workflow](https://github.com/chesun/claude-research-workflow) — 2026-06-22
